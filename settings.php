...
if (!password_verify($general_current_pass_var,$_SESSION['Password'])) {
    ...
    if (empty($fullname_var) or empty($username_var) or empty($email_var)) {
        ...
        if(strpos($username_var, ' ') !== false || preg_match('/[\'^Â£$%&*()}{@#~?><>,.|=+Â¬-]/', $username_var) || !preg_match('/[A-Za-z0-9]+/', $username_var)) {
            $general_save_result =  "
...
<head>
    <title>Account settings | BeU - Social Network For High School Students</title>
     <meta charset="UTF-8">
...
<div id="Language" class="tabcontent" style="height: auto;">
    ...
    <form action="" method="post">
        ...
        <option <?php if($_SESSION['language'] == "Ø§Ù„Ø¹Ø±Ø¨ÙŠØ©"){ echo "selected";} ?> >Ø§Ù„Ø¹Ø±Ø¨ÙŠØ©</option>
        </select>
        </p>
        <div style="background: #e9ebee; border-radius: 3px; padding: 15px;">
